"""HTTP-сервис для сайта WIXYEZ Metro Shop и API заявок."""
...
def build_api_router() -> APIRouter:
    ...
    async def api_create_invoice(body: InvoiceBody) -> dict:
        ...
        if not bot:
            raise HTTPException(
                status_code=503,
                detail="Укажите TELEGRAM_BOT_USERNAME в .env для продолжения заказа.",
            )
        start_payload = f"order_{body.product_id}_{request_id}"
        checkout = f"https://t.me/{bot}?start={start_payload}"
        return {
            "checkout_url": checkout,
            "reference_id": request_id,
            "amount": product.amount,
            "currency": product.currency,
            "note": f"{product.game_id_label}: {body.account[:120]}",
        }
