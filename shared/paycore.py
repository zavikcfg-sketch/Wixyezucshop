"""
PayCore (Corefy) Public API — создание инвойса и редирект на hpp_url.
Документация: https://merchant-docs.paycore.io/en/integration/payments/payment-page
"""
    ...
    *,
    ...
    if not settings.paycore_public_key or not settings.paycore_api_base_url:
        raise PayCoreNotConfiguredError(
            "Задайте PAYCORE_PUBLIC_KEY и PAYCORE_API_BASE_URL в .env",
        )
    if not settings.paycore_payment_service:
        raise PayCoreNotConfiguredError("Задайте PAYCORE_PAYMENT_SERVICE в .env")
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            if resp.status >= 400:
                raise PayCoreRequestError(
                    f"PayCore HTTP {resp.status}: {text[:2000]}",
                )
